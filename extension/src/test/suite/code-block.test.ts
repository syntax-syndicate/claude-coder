...
${replaceContent}`
	...
	test("applyEditBlocksToFile correctly updates the content", async () => {
		...
		assert.ok(!functionContent.includes("return [firstMessage, ...remainingMessages]"),
		...
		expectedLines.forEach(line => {
			...
			if (lineInResult) {
				assert.ok(lineInResult.startsWith("    "),
					`Line should have proper indentation: ${line}`)
			}
		})
		...
		const nextFuncIndex = resultLines.findIndex((line, idx) =>
			idx > funcStartIndex && line.includes("export function smartTruncation")
		)
		assert.ok(nextFuncIndex > funcStartIndex, 
			"Next function should exist and come after the modified function")
	})
})
